...
func respondWithError(w http.ResponseWriter, responseCode int, msg string) {
	respondWithJSON(w, responseCode, map[string]string{"error": msg})
}

func respondWithJSON(w http.ResponseWriter, resoponseCode int, data interface{}) {
	res, _ := json.Marshal(data)
	w.Header().Add("content-type", "application/json")
	w.WriteHeader(resoponseCode)
	w.Write(res)
}
...
func main() {
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprintf(w, "Test REST API with NO external dependencies!")
	})
